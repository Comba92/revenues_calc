...
export default function computeYearsRevenues(
  price: number, newCostumers: number, churnedCostumers: number, fixedCosts: number
): number[][] {
  let res: number[] = []
  ...
  // First month
  res.push(price * currentCostumers - fixedCosts)
  ...
  for(let i=1; i<12 * YEARS_TO_SHOW; ++i) {
    ...
    res.push(res[i-1] + month1 + month2 - month3 - fixedCosts)

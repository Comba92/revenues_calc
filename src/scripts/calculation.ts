...
export default function computeYearsRevenues(
  price: number, newCostumers: number, churnedCostumers: number, fixedCosts: number, variableCosts: number
): number[][] {
  let res: number[] = []
  ...
  // First month
  res.push(price*currentCostumers - fixedCosts - variableCosts*currentCostumers)
  ...
  for(let i=1; i<12 * YEARS_TO_SHOW; ++i) {
    ...
    const month4 = variableCosts * currentCostumers
    // total : 100 = x : churnedPercetnage

    res.push(res[i-1] + month1 + month2 - month3 - month4 - fixedCosts)
